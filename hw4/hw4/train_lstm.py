...
def main(options):
  ...
  rnnlm = BiLSTM(vocab_size,use_cuda = use_cuda)
  if use_cuda > 0:
    rnnlm.cuda()
  else:
    rnnlm.cpu()
  ...
  for epoch_i in range(options.epochs):
    ...
    for i, batch_i in enumerate(utils.rand.srange(len(batched_train))):
      ...
      sys_out_batch = rnnlm(train_batch, True)  # (seq_len, batch_size, vocab_size) # TODO: substitute this with your module
    ...
    for batch_i in range(len(batched_dev)):
      ...
      sys_out_batch = rnnlm(dev_batch, False)
